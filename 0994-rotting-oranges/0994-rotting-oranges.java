class Solution {
    ...
    public int orangesRotting(int[][] grid) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (grid[i][j] == 2) {
                    q.add(new Pair(i, j, 0)); //add source nodes(given already rotten oranges in grid to queue)
                } else if (grid[i][j] == 1)
                    fresh++;
            }
        }
        ...
        while (!q.isEmpty()) {
            Pair curr = q.poll();
            int r = curr.row;int c = curr.col;int t = curr.time;
            maxTime = Math.max(maxTime, t);
            for (int[] d : dirs) {
                int newRow = r + d[0]; // shift row
                int newCol = c + d[1]; // shift col
                if (newRow >= 0 && newRow < m && newCol >= 0 && newCol < n && grid[newRow][newCol] == 1) {
                    grid[newRow][newCol] = 2; //rott the adj fresh orange
                    fresh--;
